    ...
    products.forEach(product => {
      ...
      productCard.innerHTML = `
        <div class="icon">
          ...
          <button class="add-to-cart-btn" onclick="addToCart(${product.id})">Masukan Keranjang</button>
        </div>
        <h3 class="ProductName">${product.name}</h3>
        <p class="ProductPrice">Rp ${product.price.toFixed(3)}</p>
      `;
  ...
  cart.forEach(item => {
    ...
    listItem.innerHTML = `
      ...
      <div class="item-details">
        <h4>${item.name}</h4>
        <p class="item-price">Rp ${item.price.toFixed(3)}</p>
...
function getCartTotal() {
  ...
  const totalValue = parseFloat(totalText.replace('Total: Rp ', '')); 
  return totalValue;
}
